---
title: Getting started
description: Getting started with Hopper UI.
---

{/* <Callout>Hopper is compatible with React 18. Not using React, or using an older version than 18? You can still benefit from Hopper, read on to discover how.</Callout> */}

This guide will help you getting started with Hopper UI. Hopper UI is a Design System that provides a collection of components, [tokens](/tokens), and [icons](/icons) to help you build applications.

Below is the typical installation path, this assumes you are using React. If you have particluar needs you can have a look at these pages:

- [Javascript installation](/pages/getting-started/javascript)
- [Individual package installation](/pages/getting-started/individual-packages)

Run the following command in your terminal to install the Hopper packages.

<PackageInstallation library="components @hopper-ui/icons @hopper-ui/styled-system react-aria react-aria-components" mod="prod" />

After installing Hopper UI packages, you must setup a Provider at the root of your application. This is typically in your _index.tsx_ or _app.tsx_, but can vary based on your specific setup.

```jsx
import { HopperProvider } from "@hopper-ui/components";
import { createRoot } from 'react-dom/client';

const root = createRoot(document.getElementById('root'));
root.render(
  < HopperProvider >
    <App />
  </HopperProvider >
);
```

You will also need to import the Hopper styles in your project. You can do this by importing the following CSS files:

```css
@import "@hopper-ui/tokens/tokens.css";
@import "@hopper-ui/tokens/fonts.css";
```

You are now ready to use Hopper UI in your project! Need more details? Have a look at the [Individual package installation](/pages/getting-started/individual-packages) page.

## Contribute

Pull requests are welcome. For major changes, please open a [discussion](https://github.com/gsoft-inc/wl-hopper/discussions/new/choose) first to discuss what you would like to change. If you're interested in contributing, check out our [Contributing Guide](https://github.com/gsoft-inc/wl-hopper/blob/main/CONTRIBUTING.md).
