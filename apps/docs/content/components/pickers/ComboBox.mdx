...
### Custom Filtering
A combo box can overridde the default [useFilter](https://react-spectrum.adobe.com/react-aria/useFilter.html) function from RAC. This can be overridden using the `defaultFilter` prop, or by using the items prop to control the filtered list. When items is provided rather than `defaultItems`, ComboBox does no filtering of its own.
