...
Tag groups can have different variants using the `variant` prop. The one seen here is `subdued`.
...
An individual tag can be different variants using the `variant` prop.
