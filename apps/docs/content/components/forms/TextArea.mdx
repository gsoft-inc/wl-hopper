...
### Max Length Exceeded
To exceed the max length, set the `restrictMaxLength` prop to `false` – default is `true`. When the character count is displayed, it turns red if the max length is exceeded.
