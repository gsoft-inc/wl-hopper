...
### Max Length Exceeded
To exceed the max length, set the `restrictMaxLength` prop to `false` – default is `true`. When the character count is displayed, it turns red if the max length is exceeded. The `data-over-max-length` attribute is added to the text area to allow custom styling when the max length is exceeded.
