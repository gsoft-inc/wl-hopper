...
- **Use the bounding box to align with grid** - The bounding box is the vertical height of the text and is defined by the line height. That value of the line height is critical to make sure text is aligned to the 8px grid of the interface.
- **Use text-crop when a font looks off-centered** - Some fonts, such as *ABC Favorit* requires a little help to look centered. In those cases, apply a [text-crop](/guides/css/text-crop) to adjust the line height. These values are available as tokens, and are documented below. Hopper Components already apply these values when necessary.
